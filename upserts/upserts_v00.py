...
class Upsert():
    ...
    def create_file_template(self, dst: str):
        """
        it create an excel file with the table columns of self.table
        """
        columns = self.__table_column_get()
        df = pd.DataFrame(columns=columns)
        df.to_excel(dst, sheet_name=self.table, index=False, encoding='utf-8')
    ...
    def __column_names_from_element(self, root, element_name,
                                    cols_in_excelfi):
        """
