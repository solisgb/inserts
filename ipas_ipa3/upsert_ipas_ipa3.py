...
class Upsert_ipas_ipa3():
    ...
    def upsert(self, update: bool=True, check: bool=True) ->None:
        ...
        for index, row in self.data.iterrows():
            ...
            row_message = f"{index:n} {row[self.cod]} {row[self.fecha]}"
            if row1 is None:
                try:
                    cur.execute(insert,
                    ...
                    logging.append(f"{row_message} inserted", False)
                    inserted += 1
                except:
                    msg = traceback.format_exc()
                    logging.append(f"{row_message} error " +\
                                   f'inserting\n{msg}')
                    return
            else:
                if not update:
                    logging.append(f"{row_message} not updated", False)
                    continue
                try:
                    cur.execute(update,
                                (row[self.caudal_ls], row[self.error],
                                 ...
                                 row[self.medidor],
                                 row[self.cod], row[self.fecha]))
                    logging.append(f"{row_message} updated", False)
                    updated += 1
                except:
                    msg = traceback.format_exc()
                    logging.append(f"{row_message} error " +\
